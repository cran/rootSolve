...
C  === KARLINE LSODES (sparse) ADDED FOR VERSION 1.7  (2016)
C
...
C  === KARLINE ADDED THIS FOR VERSION 1.7  (2016)
...
C  JGROUP2   is called by DPREP to compute groups of Jacobian column
C           indices for use when MITER = 2. KS: added2
C  ADJLR    adjusts the length of required sparse matrix work space.
C           It is called by DPREP.
C  CNTNZU2   is called by DPREP and counts the nonzero elements in the
C           strict upper triangle of J + J-transpose, where J = df/dy. KS:2
C  DINTDY   computes an interpolated value of the y vector at t = TOUT.
C  DSTODE   is the core integrator, which does one step of the
C           integration and the associated error control.
C  DCFODE   sets all method coefficients and test constants.
...
C-----------------------------------------------------------------------
     ...
     1   TCRIT, TDIST, TNEXT, TOL, TOLSF, TP, SIZE, SUM, W0
      DIMENSION MORD(2)
      LOGICAL IHIT
C KARLINE: from length of MSG 60 -> 80      
      CHARACTER(LEN=80) MSG
...
C-----------------------------------------------------------------------
 ...
 628  MSG='DLSODES- RWORK length insufficient (for Subroutine DPREP).  '
      ...
      GO TO 700
 629  MSG='DLSODES- RWORK length insufficient (for Subroutine JGROUP2).'
...
C         -2  insufficient storage for JGROUP2.
...
C Compute grouping of column indices (MITER = 2). ----------------------
      ...
      CALL JGROUP2(N, IWK(IPIAN), IWK(IPJAN), MAXG, NGP, IWK(IPIGP),
...
C
 230  IPPER = -3
      CALL CNTNZU2 (N, IWK(IPIAN), IWK(IPJAN), NZSUT)
...
*DECK JGROUP
      SUBROUTINE JGROUP2 (N,IA,JA,MAXG,NGRP,IGP,JGP,INCL,JDONE,IER)
...
C Error return if not all columns were chosen (MAXG too small).---------
 ...
 80   IER = 1
      RETURN
C----------------------- End of Subroutine JGROUP2 ----------------------
      END

*DECK CNTNZU
      SUBROUTINE CNTNZU2 (N, IA, JA, NZSUT)
