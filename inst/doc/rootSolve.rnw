...
\Abstract{ \R package \rs \citep{rootSolve} includes
  \begin{itemize}
    ...
    \item Functions that find the steady-state condition of a set of
      ...
      with the ODE solvers in package \ds, which solve initial value ODEs.
      ...
      as a set of ODEs by numerical differencing.
    \item Functions calculate the Hessian and Jaobian matrix or - more
...
<<>>=
A <- matrix(nr=500,nc=500,runif(500*500))
B <- runif(500)
...
<<>>=
print(system.time(X <- multiroot(start=1:500, f=fun, jactype="fullusr",
   jacfunc=jfun)))
@
...
\section{Steady-state analysis}
  ...
  Within \R, package \ds \citep{deSolve} is designed to solve so-called
...
print(system.time(
RS<-runsteady(y=y,fun=model,parms=pars,times=c(0,1e5))
))
RS
@
...
\subsection{Steady-state solution of 3-D PDEs}
  Function \code{steady.3D} the steady-state of
  3-dimensional problems. See its help file for an example.
...
\section{Boundary value problems}
  ...
  interface. It also uses the method-of-lines approach. Another package,
  \pkg{bvpSolve} provides two totally different methods to solve boundary
